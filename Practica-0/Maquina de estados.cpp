...
bool active = false;
bool lightsEnabled = true;  // NUEVA: Controla si las luces están activas
...
int main()
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		modelTemp1 = modelTemp = Dibujar(model, glm::vec3(0.69f, 1.5f, 0.6f), glm::vec3(-1.31f, -0.5f, -6.3f), modelLoc); // cuerpo principal
		...
		model = glm::mat4(1.0f);
		glBindTexture(GL_TEXTURE_2D, negro);
		modelTemp1 = modelTemp = Dibujar(model, glm::vec3(0.7f, 0.8f, 0.6f), glm::vec3(-1.9f, -0.6f, -2.76f), modelLoc); // cuerpo principal
		glBindTexture(GL_TEXTURE_2D, gris);
		modelTemp = Dibujar(modelTemp1, glm::vec3(0.7f, 0.2f, 0.1f), glm::vec3(0.0f, 0.5f, 0.25f), modelLoc);// soporte superior
		modelTemp = Dibujar(modelTemp1, glm::vec3(0.5f, 0.5f, 0.005f), glm::vec3(0.0f, -0.1f, -0.31f), modelLoc);// Puerta inferior
		modelTemp = Dibujar(modelTemp1, glm::vec3(0.15f, 0.07f, 0.05f), glm::vec3(0.2f, 0.3f, -0.31f), modelLoc); // manija 1
		modelTemp = Dibujar(modelTemp1, glm::vec3(0.15f, 0.07f, 0.05f), glm::vec3(-0.2f, 0.3f, -0.31f), modelLoc); // manija 2
		glBindTexture(GL_TEXTURE_2D, rojo);
		modelTemp = Dibujar(modelTemp1, glm::vec3(0.13f, 0.05f, 0.13f), glm::vec3(0.2f, 0.4f, 0.08f), modelLoc); //parrilla 1 
		modelTemp = Dibujar(modelTemp1, glm::vec3(0.13f, 0.05f, 0.13f), glm::vec3(-0.2f, 0.4f, 0.08f), modelLoc); //parrilla 2 
		modelTemp = Dibujar(modelTemp1, glm::vec3(0.13f, 0.05f, 0.13f), glm::vec3(0.2f, 0.4f, -0.15f), modelLoc); //parrilla 3 
		modelTemp = Dibujar(modelTemp1, glm::vec3(0.13f, 0.05f, 0.13f), glm::vec3(-0.2f, 0.4f, -0.15f), modelLoc); ////parrilla 4


		//planta en maceta
		model = glm::mat4(1.0f);
		glBindTexture(GL_TEXTURE_2D, marron);		
		modelTemp1 = modelTemp = Dibujar(model, glm::vec3(0.3f, 0.4f, 0.3f), glm::vec3(-1.0f, -0.8f, -1.9f), modelLoc); // maceta
		glBindTexture(GL_TEXTURE_2D, tronco);
		modelTemp = Dibujar(modelTemp1, glm::vec3(0.09f, 1.5f, 0.09f), glm::vec3(0.0f, 0.2f, 0.0f), modelLoc); //tallo planta
		...
		modelTemp = Dibujar(modelTemp1, glm::vec3(0.25f, 0.15f, 0.05f), glm::vec3(0.0f, 0.5f, 0.0f), modelLoc); //hoja 1
		modelTemp = Dibujar(modelTemp1, glm::vec3(0.25f, 0.15f, 0.05f), glm::vec3(0.0f, 0.3f, 0.0f), modelLoc); //hoja 2


		//Cuadro fotografico 
		model = glm::mat4(1.0f);
		glBindTexture(GL_TEXTURE_2D, marco);
		modelTemp1 = modelTemp = Dibujar(model, glm::vec3(0.05f, 0.4f, 0.55f), glm::vec3(4.8f, 0.4f, 3.5f), modelLoc); //marco
		glBindTexture(GL_TEXTURE_2D, foto);
		modelTemp = Dibujar(modelTemp1, glm::vec3(0.02f, 0.35f, 0.50f), glm::vec3(-0.05f, 0.0f, 0.0f), modelLoc); //fotografia


		// guardaplatos
		model = glm::mat4(1.0f);
		glBindTexture(GL_TEXTURE_2D, mostasa);
		modelTemp1 = modelTemp = Dibujar(model, glm::vec3(0.25f, 0.8f, 0.7f), glm::vec3(-3.4f, 0.4f, -3.2f), modelLoc); //cajon
		glBindTexture(GL_TEXTURE_2D, marron);
		modelTemp = Dibujar(modelTemp1, glm::vec3(0.02f, 0.7f, 0.25f), glm::vec3(0.18f, 0.0f, 0.15f), modelLoc); //puerta 1
		modelTemp = Dibujar(modelTemp1, glm::vec3(0.02f, 0.7f, 0.25f), glm::vec3(0.18f, 0.0f, -0.15f), modelLoc); //puerta 2 
		glBindTexture(GL_TEXTURE_2D, negro);
		modelTemp = Dibujar(modelTemp1, glm::vec3(0.05f, 0.15f, 0.05f), glm::vec3(0.18f, 0.0f, 0.1f), modelLoc);// Mango puerta 
		modelTemp = Dibujar(modelTemp1, glm::vec3(0.05f, 0.15f, 0.05f), glm::vec3(0.18f, 0.0f, -0.1f), modelLoc);// Mango puerta 
...
void Animation() {
	...
	if (AnimCarro)
	{
		rotCarro += 0.1f;
			...
			tamllanta = 0.05f;
			// 2. Comprobar si ha pasado el intervalo deseado (ej. 0.5 segundos)
			if (timer >= INTERVALO_CAMBIO)
			{
				// Aplicar el cambio a la rotación (incremento en un valor fijo)
				tamllanta = -0.05f; 
